...
// Opaque dark core that fills the hollow interior so the gaps between cubelets
// reveal a solid inside instead of the starfield behind the cube. Sized just
// inside the shell (see frameCubeForSize) so it stays hidden except through the
// gaps, and is never pointer-picked.
const core = new THREE.Mesh(
  new THREE.BoxGeometry(1, 1, 1),
  new THREE.MeshStandardMaterial({ color: 0x0b0e18, roughness: 0.7, metalness: 0.05 })
);
core.raycast = () => {};
...
function frameCubeForSize() {
  ...
  const coreHalf = extent - 0.12;
  core.scale.setScalar(coreHalf * 2);
  const bottom = -extent;
